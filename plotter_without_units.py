...
def plot_filled_contours_no_zero(is_imperial_layer,ax, ax_legend, lon, lat, data, 
    ...
    # Style the colorbar
    cbar.ax.tick_params(labelsize=7, pad=2, direction='out', length=6, width=1)
...
def plot_city_names(ax, m, short_name, city_file='app/config/pac_names.json'):
...
print('is imperial. '+ str(is_imperial_layer))
...
if plot_type == "contourf":
    ...
    cs, cbar = plot_filled_contours(is_imperial_layer,ax=ax2, ax_legend=ax_legend, lon=lon, lat=lat, data=data_extract,\
    ...
    cs, cbar = plot_filled_contours_no_zero(is_imperial_layer,ax=ax2, ax_legend=ax_legend, lon=lon, lat=lat, data=data_extract,\
        min_color_plot=min_color_plot, max_color_plot=max_color_plot, steps=steps, cmap_name=cmap_name, units=units
    )
    ...
    local_path=True, local_path_str=local_file_name)
    step = 10
    if int(region) == 1:
        step = 30
    ...
    local_path=check_local, local_path_str=local_file_name)
    cs, cbar = plot_climatology(is_imperial_layer,dap_url,time,ax=ax2, ax_legend=ax_legend, lon=lon, lat=lat, data=data_extract,\
        min_color_plot=min_color_plot, max_color_plot=max_color_plot, steps=steps, cmap_name=cmap_name, units=units,
        local_path=check_local, local_path_str=local_file_name
    )
    ...
    if int(splitt[1]) == int(region):
        ...
        plot_ugrid_mesh(
            ...
            get_custom_colormap=get_custom_colormap, # your function here
            extract_from_dap_ugrid=extract_from_dap_ugrid, # your function here
